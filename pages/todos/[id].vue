    ...
    <div v-else-if="todo" class="space-y-6">
        ...
        <div class="flex-1">
          ...
          <div class="flex flex-wrap items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
            ...
            <div class="flex items-center gap-1">
              ...
              <span>Update {{ formatDate(todo.updated_at) }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          <template #content>
            <div class="text-center">
              <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">Prioritas</div>
              <Badge
          ...
          <template #content>
            <div class="text-center">
              ...
              <div class="text-xs text-gray-500">
                {{ todo.assignments_summary?.completed_count || 0 }}/{{ todo.assignments_summary?.total_count || 0 }} selesai
              </div>
            </div>
          </template>
        </Card>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <template #content>
              <div v-if="todo.assignments_summary" class="space-y-4">
                <!-- Progress Overview -->
                <div>
                  <div class="flex justify-between text-sm mb-2">
                    <span>Progress Keseluruhan</span>
                    <span>{{ todo.completion_percentage }}%</span>
                  </div>
                  <ProgressBar
                ...
                <!-- User Assignments -->
                <div v-if="todo.assignments_summary.users.length > 0">
                      ...
                      <div class="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div class="text-xs text-gray-500 dark:text-gray-400">
                            Assigned {{ formatDate(todo.assigned_at) }}
                          </div>
                        </div>
                      </div>
                      <div class="flex items-center gap-2">
                        ...
                        />
                        <Dropdown
                          v-if="canUpdateAssignmentStatus(user)"
                          :options="statusOptions"
                          optionLabel="label"
                          optionValue="value"
                          placeholder="Update"
                          size="small"
                          @change="updateAssignmentStatus(user.id, $event)"
                        />
                      </div>
                    </div>
                  </div>
                ...
                <!-- Role Assignments -->
                <div v-if="todo.assignments_summary.roles.length > 0">
                  <h4 class="font-medium text-gray-900 dark:text-white mb-3">
                    Role Assignments
                  </h4>
                  <div class="space-y-3">
                    ...
                    >
                      <div class="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div class="text-xs text-gray-500 dark:text-gray-400">
                            {{ role.user_count }} user terassign
                          </div>
                        </div>
                      </div>
                      <Badge
                ...
                <!-- No Assignments -->
                <div v-if="!todo.assignments_summary.users.length && !todo.assignments_summary.roles.length" class="text-center py-8">
        ...
        <div class="space-y-6">
          ...
          <Card v-if="todo.category">
            ...
            <template #content>
              <div class="flex items-center gap-3">
                ...
                >
                  <Icon :name="todo.category.icon || 'lucide:tag'" class="w-5 h-5" />
                </div>
                <div>
                  <div class="font-medium text-gray-900 dark:text-white">
                    {{ todo.category.name }}
                  </div>
                  <div v-if="todo.category.description" class="text-xs text-gray-500 dark:text-gray-400">
          ...
          <Card v-if="todo.due_date">
            ...
            <template #content>
              <div class="space-y-2">
                ...
                <div v-if="todo.due_date_days_left !== undefined" class="text-sm">
                  <span
          ...
          <Card>
            ...
            <template #content>
              <div class="flex items-center gap-2">
                <Icon
                  :name="todo.is_private ? 'lucide:lock' : 'lucide:unlock'"
                  class="w-4 h-4"
                />
                <span>{{ todo.is_private ? 'Private' : 'Public' }}</span>
              </div>
              <p v-if="todo.is_private" class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Hanya Anda dan user yang diassign yang bisa melihat todo ini
              </p>
            </template>
          </Card>
        </div>
...
const getTodo = async () => {
  ...
  try {
    const response = await client(`/api/todos/${route.params.id}`)
...
const updateAssignmentStatus = async (userId: number, newStatus: string) => {
  try {
    ...
    const assignment = todo.value?.assignments?.find((a: any) =>
    ...
    await client(`/api/todos/${todo.value.id}/assignments/${assignment.id}/status`, {
      method: 'PUT',
