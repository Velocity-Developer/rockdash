<template>
    ...
    <div class="flex flex-col md:flex-row justify-start gap-2 md:items-center">
        <Button
            type="button"
            @click="
                openDialog('add', { tgl: filters.bulan, bank: filters.bank })
            "
            size="small"
            class="capitalize"
        >
    ...
    <div class="overflow-x-auto mb-5 pb-2">
        <div class="flex gap-4">
            <div
                @click="dialogFormSaldo = true"
                class="min-w-[200px] group relative cursor-pointer py-2 px-3 bg-gray-50 dark:bg-gray-800 border border-gray dark:border-gray-800 rounded"
            >
                ...
                <div
                    class="absolute w-full text-sm group-hover:block hidden bottom-0 end-0 bg-amber-100 dark:bg-amber-700 px-4 py-1 shadow-lg rounded border"
                >
            ...
            <div
                class="min-w-[200px] py-2 px-3 bg-sky-100 dark:bg-sky-800 border border-sky-500 dark:border-sky-800 rounded"
            >
                <div class="text-xs mb-2 flex items-center gap-1">
            ...
            <div
                class="min-w-[200px] py-2 px-3 bg-red-50 dark:bg-red-900 border border-red-600 dark:border-red-800 rounded"
            >
                <div class="text-xs mb-2 flex items-center gap-1">
            ...
            <div
                class="min-w-[200px] py-2 px-3 bg-green-50 dark:bg-green-900 border border-green-600 dark:border-green-800 rounded"
            >
                <div class="text-xs mb-2 flex items-center gap-1">
                ...
                <Skeleton v-if="status == 'pending'" class="w-full mt-2 h-20" />
                <div v-else class="font-bold text-end">
                    {{ formatMoney(data?.saldo_saatini, filters.bank) }}
                </div>
            </div>
        </div>
    </div>
    ...
    <Dialog
        ...
        modal
        :header="actionDialog == 'add' ? 'Tambah Transaksi' : 'Edit Transaksi'"
        :style="{ width: '50rem' }"
    ...
    <Dialog
        v-model:visible="dialogFormSaldo"
        modal
        :header="'Atur Saldo ' + getBankLabel(filters.bank)"
        :style="{ width: '30rem' }"
        :breakpoints="{ '1199px': '75vw', '575px': '90vw' }"
    >
        <BankTransaksiFormSaldo
            :bulan="filters.bulan"
            :bank="filters.bank"
            @update="refresh()"
        />
...
const exportExcel = async () => {
    ...
    try {
        ...
        XLSX.writeFile(
            workbook,
            `Transaksi Bank ${filters.bank} ${filters.bulan}.xlsx`,
        );
    } catch (error) {
