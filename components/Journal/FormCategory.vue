...
const handleSubmit = async () => {
  ...
  if(props.action === 'add') {
    try {
      await client('/api/journal_category', {
        method: 'POST',
        body: form
      })
      emit('update')
      toast.add({
        severity: 'success',
        summary: 'Berhasil!',
        detail: 'Kategori jurnal berhasil ditambahkan',
        life: 3000
      });
    } catch (error) {
      console.log(error);
      toast.add({
        severity: 'error',
        summary: 'Gagal!',
        detail: 'Kategori jurnal gagal ditambahkan',
        life: 3000
      });
    }
  } else {
    try {
      await client('/api/journal_category/' + props.item.id, {
        method: 'PUT',
        body: form
      })
      emit('update')
      toast.add({
        severity: 'success',
        summary: 'Berhasil!',
        detail: 'Kategori jurnal berhasil diupdate',
        life: 3000
      });
    } catch (error) {
      console.log(error);
      toast.add({
        severity: 'error',
        summary: 'Gagal!',
        detail: 'Kategori jurnal gagal diupdate',
        life: 3000
      });
    }
  }
